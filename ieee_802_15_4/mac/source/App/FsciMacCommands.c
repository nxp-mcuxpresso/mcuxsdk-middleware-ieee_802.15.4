...
#define pClientPacket ((clientPacket_t*)pData)
    ...
    {
        pMcpsReq = MSG_Alloc(sizeof(nwkToMcpsMessage_t) + 
    ...
    if (gSuccess_c != NWK_MCPS_SapHandler(pMcpsReq, fsciGetMacInstanceId(interfaceId)))
    {
        MSG_Free(pMcpsReq);
    }
#undef pClientPacket
...
#define pClientPacket ((clientPacket_t*)pData)
    ...
    {
        FSCI_transmitFormatedPacket(pClientPacket, fsciInterface);
    }
    else
    {
        MEM_BufferFree(pClientPacket);
    }
...
/*! *********************************************************************************
* \brief   This is the handler function for MLME Requests received over the
           serial interface
*
* \param[in] pData pointer to data location
...
{
#define pClientPacket ((clientPacket_t*)pData)
    mlmeMessage_t *pMlmeReq = MSG_Alloc(sizeof(mlmeMessage_t));
    uint8_t        *p = &pClientPacket->structured.payload[0];
    nwkMessage_t syncCnf;
    ...
    case mFsciNwkMlmeGetReq_c:
        pMlmeReq->msgType = gMlmeGetReq_c;
        pMlmeReq->msgData.getReq.pibAttribute      = pClientPacket->structured.payload[0];
        pMlmeReq->msgData.getReq.pibAttributeIndex = pClientPacket->structured.payload[1];
        break;

    case mFsciNwkMlmeSetReq_c:
        pMlmeReq->msgType = gMlmeSetReq_c;
        pMlmeReq->msgData.setReq.pibAttribute      = pClientPacket->structured.payload[0];
        pMlmeReq->msgData.setReq.pibAttributeIndex = pClientPacket->structured.payload[1];
        pMlmeReq->msgData.setReq.pibAttributeValue = &pClientPacket->structured.payload[2];
        break;

    case mFsciNwkMlmeResetReq_c:
        pMlmeReq->msgType = gMlmeResetReq_c;
        pMlmeReq->msgData.resetReq.setDefaultPIB = pClientPacket->structured.payload[0];
        break;
#if gTschSupport_d
    case mFsciNwkMlmeSetSlotframeReq_c:
        pMlmeReq->msgType = gMlmeSetSlotframeReq_c;
        pMlmeReq->msgData.setSlotframeReq.slotframeHandle = *p++;
        pMlmeReq->msgData.setSlotframeReq.operation = (macSetSlotframeOp_t)*p++;
        FLib_MemCpy(&pMlmeReq->msgData.setSlotframeReq.size, p, sizeof(uint16_t));
        p += sizeof(uint16_t);
        break;
        
    case mFsciNwkMlmeSetLinkReq_c:
        pMlmeReq->msgType = gMlmeSetLinkReq_c;
        pMlmeReq->msgData.setLinkReq.operation = (macSetLinkOp_t)*p++;
        FLib_MemCpy(&pMlmeReq->msgData.setLinkReq.linkHandle, p, sizeof(uint16_t));
        p += sizeof(uint16_t);       
        pMlmeReq->msgData.setLinkReq.slotframeHandle = *p++;
        FLib_MemCpy(&pMlmeReq->msgData.setLinkReq.timeslot, p, sizeof(uint16_t));
        p += sizeof(uint16_t);         
        FLib_MemCpy(&pMlmeReq->msgData.setLinkReq.channelOffset, p, sizeof(uint16_t));
        p += sizeof(uint16_t);
        FLib_MemCpy(&pMlmeReq->msgData.setLinkReq.linkOptions, p, sizeof(uint8_t));
        p += sizeof(uint8_t);        
        pMlmeReq->msgData.setLinkReq.linkType = (macLinkType_t)*p++;
        FLib_MemCpy(&pMlmeReq->msgData.setLinkReq.nodeAddr, p, sizeof(uint16_t));
        p += sizeof(uint16_t);    
        break;

    case mFsciNwkMlmeTschModeReq_c:
        pMlmeReq->msgType = gMlmeTschModeReq_c;
        pMlmeReq->msgData.tschModeReq.tschMode = (macTschMode_t)*p++;
        break;
        
    case mFsciNwkMlmeKeepAliveReq_c:
        pMlmeReq->msgType = gMlmeKeepAliveReq_c;
        FLib_MemCpy(&pMlmeReq->msgData.keepAliveReq.dstAddr, p, sizeof(uint16_t));
        p += sizeof(uint16_t);        
        FLib_MemCpy(&pMlmeReq->msgData.keepAliveReq.keepAlivePeriod, p, sizeof(uint16_t));
        p += sizeof(uint16_t);
        break;        
    ...
    case mFsciNwkMlmeAssociateReq_c:
        pMlmeReq->msgType = gMlmeAssociateReq_c;
        pMlmeReq->msgData.associateReq.channelPage = gDefaultChannelPageId_c;
        FLib_MemCpy(&pMlmeReq->msgData.associateReq.coordAddress, p, 8);
        p += 8;
        FLib_MemCpy( &pMlmeReq->msgData.associateReq.coordPanId,
                     p, sizeof(uint16_t));
        p += sizeof(uint16_t);
        pMlmeReq->msgData.associateReq.coordAddrMode  = (addrModeType_t)*p++;
        pMlmeReq->msgData.associateReq.logicalChannel = *p++;
#if gMacSecurityEnable_d
        pMlmeReq->msgData.associateReq.securityLevel  = (macSecurityLevel_t)*p++;
        pMlmeReq->msgData.associateReq.keyIdMode      = (keyIdModeType_t)*p++;
        FLib_MemCpy(&pMlmeReq->msgData.associateReq.keySource, p, 8);
        p += 8;
        pMlmeReq->msgData.associateReq.keyIndex = *p++;
#else
        pMlmeReq->msgData.associateReq.securityLevel  = gMacSecurityNone_c;
        p += 11;
#endif
        pMlmeReq->msgData.associateReq.capabilityInfo = *p++;
        break;

#if gMacCoordinatorCapability_d
    case mFsciNwkMlmeAssociateRes_c:
        pMlmeReq->msgType = gMlmeAssociateRes_c;
        FLib_MemCpy(&pMlmeReq->msgData.associateRes.deviceAddress, p, 8);
        p += 8;
        FLib_MemCpy( &pMlmeReq->msgData.associateRes.assocShortAddress, p,
                    sizeof(uint16_t));
        p += sizeof(uint16_t);
#if gMacSecurityEnable_d
        pMlmeReq->msgData.associateRes.securityLevel  = (macSecurityLevel_t)*p++;
        pMlmeReq->msgData.associateRes.keyIdMode      = (keyIdModeType_t)*p++;
        FLib_MemCpy(&pMlmeReq->msgData.associateRes.keySource, p, 8);
        p += 8;
        pMlmeReq->msgData.associateRes.keyIndex = *p++;
#else
        pMlmeReq->msgData.associateRes.securityLevel  = gMacSecurityNone_c;
        p += 11;
#endif
        pMlmeReq->msgData.associateRes.status   = (resultType_t)*p++;
        break;
#endif

    case mFsciNwkMlmeDisassociateReq_c:
        pMlmeReq->msgType = gMlmeDisassociateReq_c;
        FLib_MemCpy(&pMlmeReq->msgData.disassociateReq.deviceAddress, p, 8);
        p += 8;
        FLib_MemCpy( &pMlmeReq->msgData.disassociateReq.devicePanId, p,
                     sizeof(uint16_t));
        p += sizeof(uint16_t);
        pMlmeReq->msgData.disassociateReq.deviceAddrMode  = (addrModeType_t)*p++;
        pMlmeReq->msgData.disassociateReq.disassociateReason = (macDisassociateReason_t)*p++;
        pMlmeReq->msgData.disassociateReq.txIndirect = *p++;
#if gMacSecurityEnable_d
        pMlmeReq->msgData.disassociateReq.securityLevel = (macSecurityLevel_t)*p++;
        pMlmeReq->msgData.disassociateReq.keyIdMode     = (keyIdModeType_t)*p++;
        FLib_MemCpy(&pMlmeReq->msgData.disassociateReq.keySource, p, 8);
        p += 8;
        pMlmeReq->msgData.disassociateReq.keyIndex = *p++;
#else
        pMlmeReq->msgData.disassociateReq.securityLevel = gMacSecurityNone_c;
#endif
        break;
#endif /* #if gMacUseAssociation_d */
    ...
    case mFsciNwkMlmeGtsReq_c:
        pMlmeReq->msgType = gMlmeGtsReq_c;
        *(uint8_t*)(&pMlmeReq->msgData.gtsReq.gtsCharacteristics) = *p++;
#if gMacSecurityEnable_d
        pMlmeReq->msgData.gtsReq.securityLevel      = (macSecurityLevel_t)*p++;
        pMlmeReq->msgData.gtsReq.keyIdMode          = (keyIdModeType_t)*p++;
        FLib_MemCpy(&pMlmeReq->msgData.gtsReq.keySource, p,
                    sizeof(pMlmeReq->msgData.gtsReq.keySource));
        p += sizeof(pMlmeReq->msgData.gtsReq.keySource);
        pMlmeReq->msgData.gtsReq.keyIndex = *p;
#else
        pMlmeReq->msgData.gtsReq.securityLevel      = gMacSecurityNone_c;
#endif
        break;
#endif /* #if gGtsSupport_d */

#if gMacCoordinatorCapability_d
    case mFsciNwkMlmeOrphanRes_c:
        pMlmeReq->msgType = gMlmeOrphanRes_c;
        FLib_MemCpy(&pMlmeReq->msgData.orphanRes.orphanAddress, p, 8);
        p += 8;
        FLib_MemCpy( &pMlmeReq->msgData.orphanRes.shortAddress, p,
                    sizeof(uint16_t));
        p += sizeof(uint16_t);
        pMlmeReq->msgData.orphanRes.associatedMember = *p++;
#if gMacSecurityEnable_d
        pMlmeReq->msgData.orphanRes.securityLevel = (macSecurityLevel_t)*p++;
        pMlmeReq->msgData.orphanRes.keyIdMode = (keyIdModeType_t)*p++;
        FLib_MemCpy(&pMlmeReq->msgData.orphanRes.keySource, p, 8);
        p += 8;
        pMlmeReq->msgData.orphanRes.keyIndex = *p++;
#else
        pMlmeReq->msgData.orphanRes.securityLevel = gMacSecurityNone_c;
#endif
        break;
#endif /* #if gMacCoordinatorCapability_d */

#if gMacUseRxEnableRequest_d
    case mFsciNwkMlmeRxEnableReq_c:
        pMlmeReq->msgType = gMlmeRxEnableReq_c;
        pMlmeReq->msgData.rxEnableReq.deferPermit = *p++;
        pMlmeReq->msgData.rxEnableReq.rxOnTime = 0;
        pMlmeReq->msgData.rxEnableReq.rxOnDuration = 0;
        FLib_MemCpy(&pMlmeReq->msgData.rxEnableReq.rxOnTime, p, 3);
        p += 3;
        FLib_MemCpy(&pMlmeReq->msgData.rxEnableReq.rxOnDuration, p, 3);
#ifdef gMAC2011_d
        p += 3;
        pMlmeReq->msgData.rxEnableReq.rangingRxControl = (macRangingRxControl_t)*p;
#endif
        break;
#endif /* #if gMacUseRxEnableRequest_d */

    case mFsciNwkMlmeScanReq_c:
        pMlmeReq->msgType = gMlmeScanReq_c;
        pMlmeReq->msgData.scanReq.scanType = (macScanType_t)*p++;
        FLib_MemCpy( &pMlmeReq->msgData.scanReq.scanChannels, p, sizeof(channelMask_t) );
        p += sizeof(channelMask_t);
        pMlmeReq->msgData.scanReq.scanDuration = *p++;
#if gMacSecurityEnable_d
        pMlmeReq->msgData.scanReq.securityLevel = (macSecurityLevel_t)*p++;
        pMlmeReq->msgData.scanReq.keyIdMode = (keyIdModeType_t)*p++;
        FLib_MemCpy(&pMlmeReq->msgData.scanReq.keySource, p, 8);
        p += 8;
        pMlmeReq->msgData.scanReq.keyIndex = *p++;
#else
        pMlmeReq->msgData.scanReq.securityLevel = gMacSecurityNone_c;
#endif
        break;

#if gMacUseMlmeStart_d && gMacCoordinatorCapability_d
    case mFsciNwkMlmeStartReq_c:
        pMlmeReq->msgType = gMlmeStartReq_c;
        pMlmeReq->msgData.startReq.channelPage = gDefaultChannelPageId_c;
        FLib_MemCpy( &pMlmeReq->msgData.startReq.panId, p,
                    sizeof(uint16_t));
        p += sizeof(uint16_t);
        pMlmeReq->msgData.startReq.logicalChannel = *p++;
        FLib_MemCpy( &pMlmeReq->msgData.startReq.startTime, p, sizeof(uint32_t) );
        p += sizeof(uint32_t);
        pMlmeReq->msgData.startReq.beaconOrder = *p++;
        pMlmeReq->msgData.startReq.superframeOrder = *p++;
        pMlmeReq->msgData.startReq.panCoordinator = *p++;
        pMlmeReq->msgData.startReq.batteryLifeExtension = *p++;
        pMlmeReq->msgData.startReq.coordRealignment = *p++;
#if gMacSecurityEnable_d
        pMlmeReq->msgData.startReq.coordRealignSecurityLevel = (macSecurityLevel_t)*p++;
        pMlmeReq->msgData.startReq.coordRealignKeyIdMode = (keyIdModeType_t)*p++;
        FLib_MemCpy(&pMlmeReq->msgData.startReq.coordRealignKeySource, p, 8);
        p += 8;
        pMlmeReq->msgData.startReq.coordRealignKeyIndex = *p++;
        pMlmeReq->msgData.startReq.beaconSecurityLevel = (macSecurityLevel_t)*p++;
        pMlmeReq->msgData.startReq.beaconKeyIdMode = (keyIdModeType_t)*p++;
        FLib_MemCpy(&pMlmeReq->msgData.startReq.beaconKeySource, p, 8);
        p += 8;
        pMlmeReq->msgData.startReq.beaconKeyIndex = *p++;
#else
        pMlmeReq->msgData.startReq.coordRealignSecurityLevel = gMacSecurityNone_c;
#endif
        break;
#endif /* #if gMacUseMlmeStart_d && gMacCoordinatorCapability_d */

#if gBeaconEnabledSupport_d
    case mFsciNwkMlmeSyncReq_c:
        pMlmeReq->msgType = gMlmeSyncReq_c;
        pMlmeReq->msgData.syncReq.channelPage = gDefaultChannelPageId_c;
        pMlmeReq->msgData.syncReq.logicalChannel = *p++;
        pMlmeReq->msgData.syncReq.trackBeacon    = *p;
        break;
#endif /* #if gBeaconEnabledSupport_d */

    case mFsciNwkMlmePollReq_c:
        pMlmeReq->msgType = gMlmePollReq_c;
        FLib_MemCpy(&pMlmeReq->msgData.pollReq.coordAddress, p, sizeof(pMlmeReq->msgData.pollReq.coordAddress));
        p += sizeof(pMlmeReq->msgData.pollReq.coordAddress);
        FLib_MemCpy( &pMlmeReq->msgData.pollReq.coordPanId, p,
                     sizeof(pMlmeReq->msgData.pollReq.coordPanId));
        p += sizeof(pMlmeReq->msgData.pollReq.coordPanId);
        pMlmeReq->msgData.pollReq.coordAddrMode = (addrModeType_t)*p++;
#if gMacSecurityEnable_d
        pMlmeReq->msgData.pollReq.securityLevel = (macSecurityLevel_t)*p++;
        pMlmeReq->msgData.pollReq.keyIdMode     = (keyIdModeType_t)*p++;
        FLib_MemCpy(&pMlmeReq->msgData.pollReq.keySource, p,
                    sizeof(pMlmeReq->msgData.pollReq.keySource));
        p += sizeof(pMlmeReq->msgData.pollReq.keySource);
        pMlmeReq->msgData.pollReq.keyIndex = *p++;
#else
        pMlmeReq->msgData.pollReq.securityLevel = gMacSecurityNone_c;
#endif
        break;
        
    case mFsciNwkMlmeBeaconReq_c:
        pMlmeReq->msgType = gMlmeBeaconReq_c;
        pMlmeReq->msgData.beaconReq.beaconType = (beaconType_t)*p++;
        pMlmeReq->msgData.beaconReq.channel = *p++;
        pMlmeReq->msgData.beaconReq.channelPage = (channelPageId_t)*p++;
        pMlmeReq->msgData.beaconReq.superframeOrder = *p++;
#if gMacSecurityEnable_d
        pMlmeReq->msgData.beaconReq.beaconSecurityLevel = (macSecurityLevel_t)*p++;
        pMlmeReq->msgData.beaconReq.beaconKeyIdMode = (keyIdModeType_t)*p++;
        FLib_MemCpy(&pMlmeReq->msgData.beaconReq.beaconKeySource, p, sizeof(uint64_t));
        p += sizeof(uint64_t);
        pMlmeReq->msgData.beaconReq.beaconKeyIndex = *p++;
#else
        pMlmeReq->msgData.beaconReq.beaconSecurityLevel = gMacSecurityNone_c;
        p += 11;
#endif
        pMlmeReq->msgData.beaconReq.dstAddrMode = (addrModeType_t)*p++;
        FLib_MemCpy(&pMlmeReq->msgData.beaconReq.dstAddr, p, sizeof(uint64_t));
        p += sizeof(uint64_t);
        pMlmeReq->msgData.beaconReq.bsnSuppression = *p++;
        break;
    ...
    if( gSuccess_c != NWK_MLME_SapHandler(pMlmeReq, &syncCnf, fsciGetMacInstanceId(interfaceId)))
    {
        MSG_Free(pMlmeReq);
    }
    else
    {
        if (syncCnf.msgType == gMlmeGetCnf_c)
        {
            /* Make sure to free the pib attribute */
            MEM_BufferFree(syncCnf.msgData.getCnf.pibAttributeValue);
        }
    }
...
#if gPhyEfpEnabled
        *p++ = pMscpCnf->msgData.dataCnf.efpState;
#endif
        break;
...
#else
        size += sizeof(mlmeGetCnf_t) + mlmeGetSizeOfPIB(pMlmeCnf->msgData.getCnf.pibAttribute);
#endif
        break;
...
#undef pSrc
    ...
    case gMlmeGetCnf_c:          /*84 05*/
        ...
        *p++ = pMlmeCnf->msgData.getCnf.pibAttribute;
        *p++ = pMlmeCnf->msgData.getCnf.pibAttributeIndex;
#if gFsciHost_802_15_4_c
        if( (gMPibBeaconPayload_c == pMlmeCnf->msgData.getCnf.pibAttribute) 
#if gTschSupport_d
         || (gMPibHoppingSequenceList_c == pMlmeCnf->msgData.getCnf.pibAttribute) 
#endif
          )
        {
            size = gFsciMaxPayloadLen_c - sizeof(mlmeGetCnf_t);
...
#else          
        size = mlmeGetSizeOfPIB(pMlmeCnf->msgData.getCnf.pibAttribute);
#endif
        FLib_MemCpy( p, &size, sizeof(size) );
        p += sizeof(size);
        FLib_MemCpy(p, pMlmeCnf->msgData.getCnf.pibAttributeValue, size);
        p += size;
        break;
...
#undef pSrc
    ...
    case gMlmeSetCnf_c:          /*84 0D*/
        ...
        *p++ = (uint8_t)((uint32_t)param); /*Status of Sync request*/
        *p++ = pMlmeCnf->msgData.setCnf.pibAttribute;
        *p++ = pMlmeCnf->msgData.setCnf.pibAttributeIndex;
        break;
#if gMacUseMlmeStart_d && gMacCoordinatorCapability_d
