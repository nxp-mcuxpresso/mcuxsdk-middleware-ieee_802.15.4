...
#if (defined(GP_TX_DELAY_ENABLE) && (GP_TX_DELAY_ENABLE == 1))
    uint32_t                txStart;    /*!< The time (in symbols), at which the data should be transmitted. This is a 24-bit value. */
#endif
...
typedef MAC_STRUCT mlmePollNotifyInd_tag
{
    ...
    bool_t                  sentFP;      /*!< POLL acked FP set bit */
} mlmePollNotifyInd_t;
...
typedef MAC_STRUCT macMsgHeader_tag{
    uint8_t             msgType;        /* macMessageId_t */
    uint8_t             mac_id;
} macMsgHeader_t;
...
{
    uint8_t             msgType; /*!< The id of the MAC massage */
    uint8_t             mac_id;
    MAC_UNION
    {
        mcpsDataReq_t       dataReq;
        mcpsPurgeReq_t      purgeReq;
        mcpsPurgeIndQueueReq_t purgeIndQueueReq;
    } msgData;
} nwkToMcpsMessage_t;
...
*
********************************************************************************** */
resultType_t NWK_MLME_SapHandler( mlmeMessage_t*      pMsg, nwkMessage_t *pCnf, instanceId_t macInstanceId );
