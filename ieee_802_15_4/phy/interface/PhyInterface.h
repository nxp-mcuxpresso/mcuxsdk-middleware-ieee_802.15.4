...
/**
 * @brief PHY API version structure
 *
 * Contains major, minor, patch version numbers following semantic versioning.
 */
typedef struct {
    uint8_t     msg_type;
    uint8_t     ctx_id;
...
/**
 * @brief Get PHY API version information
 *
 * For standalone/NBU builds, returns compile-time version.
 * For split architectures, queries version from NBU on first call (lazy init).
 *
 * @return Pointer to static api_version_t structure, never NULL
 * @note Thread-safe. Must be called from task context in RTOS environments.
 * @note First call may block while querying NBU in split architectures.
 */
