...
static void PhyRxCallback(uint8_t *data, uint16_t len, void *param)
{
    (void)param;
...
void Phy_Init(void)
{
    ...
    if (PLATFORM_InitZbPhyInterface(PhyRxCallback, NULL) != 0)
    {
        assert(0);
    }
...
phyStatus_t MAC_PD_SapHandler(macToPdDataMessage_t *pMsg, instanceId_t phyInstance)
{
    ...
    if(PLATFORM_SendZbPhyMessage((uint8_t *)pMsg, len) != 0)
    {
        assert(0);
    }
